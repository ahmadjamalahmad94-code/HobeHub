...
The cards-subscriber request path now GENERATES a card instantly from the
marketplace offer via RADIUS (``generate_user_cards``) instead of pulling from
local inventory. These tests mirror the FakeRadiusClient style in
``tests/test_radius_match.py``:

* writes ENABLED + live  → real generated card is persisted + delivered/linked.
* correct category code  → passed through to ``generate_user_cards``.
* writes DISABLED        → graceful pending, never a fabricated card, no crash.
...
CARDS = "نظام البطاقات"  # "نظام البطاقات"
...
class FakeLiveCardClient:
    ...
    def generate_user_cards(self, category_code, count=1, *, beneficiary_id=None,
                            requested_by="", notes=""):
        self.calls.append({
...
class FakeNeverCalledClient:
    ...
    def generate_user_cards(self, *a, **k):
        self.called = True
        raise AssertionError("generate_user_cards must not be called for ineligible users")
...
def _cleanup(beneficiary_id):
    ...
    db.execute_sql(
        "DELETE FROM radius_pending_actions WHERE beneficiary_id=%s", [beneficiary_id]
    )
...
def test_instant_generation_delivers_and_links_real_card(monkeypatch, client):
    ...
    try:
        ...
        # a real card was returned + delivered (not a pending action)
        ...
        # persisted + linked to the subscriber's beneficiary account
        row = db.query_one(
        ...
        # audited as a generation event WITHOUT the plaintext password
        audit = db.query_one(
...
def test_correct_category_is_forwarded_to_radius(monkeypatch, client):
    phone = _make_phone()
    bid = _insert_beneficiary(phone)
    fake = FakeLiveCardClient()
    monkeypatch.setattr(card_dispatcher, "get_radius_client", lambda: fake)
...
def test_writes_disabled_degrades_to_pending_without_fake_card(monkeypatch, client):
    ...
    try:
        ...
        # graceful: pending action, NO fabricated card, no crash
        assert result.ok is True
        ...
        # nothing was persisted as a delivered card
        assert db.query_one(
...
def test_ineligible_user_is_blocked_before_radius(monkeypatch, client):
    phone = _make_phone()
    # tawjihi is allowed half_hour only → three_hours must be blocked
